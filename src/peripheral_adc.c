...
int peripheral_adc_open(int device, int channel, peripheral_adc_h *adc)
{
	...
	/* Initialize */
	handle = (peripheral_adc_h)malloc(sizeof(struct _peripheral_adc_s));
	if (handle == NULL) {
	...
	if (ret != PERIPHERAL_ERROR_NONE) {
		_E("Failed to open adc communication with daemon, ret : %d", ret);
		free(handle);
		handle = NULL;
	}
...
int peripheral_adc_close(peripheral_adc_h adc)
{
	...
	if (ret != PERIPHERAL_ERROR_NONE) {
		_E("Failed to close adc communication with daemon, ret : %d", ret);
	}
...
int peripheral_adc_read(peripheral_adc_h adc, uint32_t *data)
{
	...
	return peripheral_interface_adc_read(adc, data);
}